...
(function(global, factory) {
  ...
  function initSyncableConnect(db, connect) {
    return function syncableConnect(
      protocolInstance,
      protocolName,
      url,
      options
        ...
        if (db._localSyncNode.isMaster) {
          ...
          return db
            ...
            .first(function(masterNode) {
              ...
              return db.observable.sendMessage(
                'connect',
                {
        ...
        var promise = new Promise$1(function(resolve, reject) {
          db.on('ready', function() {
            ...
            return db._syncNodes.get({ url: url }, function(node) {
              ...
              var connectPromise = db.syncable.connect(
                protocolName,
                url,
                options
              );
        ...
        return db._syncNodes
          ...
          .first(function(node) {
            ...
            if (node) {
              ...
              node.syncContext = new PersistedContext(
                node.id,
                node.syncContext
              );
              ...
              Dexie.Promise.resolve(
                (function() {
                  ...
                  if (options.initialUpload === false)
                    return db._changes
                      .toCollection()
                      .lastKey(function(currentRevision) {
  ...
  function initSaveToUncommittedChanges(db, node) {
    return function saveToUncommittedChanges(changes, remoteRevision) {
      return db
        .transaction('rw!', db._uncommittedChanges, function() {
          return db._uncommittedChanges.bulkAdd(
    ...
    return function applyChanges(changes) {
      ...
      changes.forEach(function(change) {
        if (!collectedChanges.hasOwnProperty(change.table)) {
          collectedChanges[change.table] = ((_a = {}),
          (_a[CREATE] = []),
          (_a[DELETE] = []),
          (_a[UPDATE] = []),
          _a);
      ...
      return db.transaction('rw', tables, function() {
        table_names.forEach(function(table_name) {
          ...
          if (deleteChangesToApply.length > 0)
            table.bulkDelete(
              deleteChangesToApply.map(function(c) {
                return c.key;
              })
            );
        });
      ...
      return db.transaction('rw!', tablesToIncludeInTrans, function() {
        ...
        return db._changes
          ...
          .then(function(lastChange) {
            ...
            node.appliedRemoteRevision = remoteRevision;
            node.remoteBaseRevisions.push({
              remote: remoteRevision,
              local: currentLocalRevision
            });
            ...
            node.save().catch(function(err) {
              console.warn(
                'Dexie.Syncable: Unable to save SyncNode after applying remote changes: ' +
                  (err.stack || err)
              );
            });
          });
  ...
  function getBaseRevisionAndMaxClientRevision(node) {
    ...
    for (var i = node.remoteBaseRevisions.length - 1; i >= 0; --i) {
      if (node.myRevision >= node.remoteBaseRevisions[i].local) {
        ...
        return {
          maxClientRevision:
            i === node.remoteBaseRevisions.length - 1
              ? Infinity
              : node.remoteBaseRevisions[i + 1].local,
  ...
  function initGetChangesSinceRevision(db, node, hasMoreToGive) {
    return function getChangesSinceRevision(
      revision,
      maxChanges,
      maxRevision,
      cb
      ...
      return db
        .transaction('r', db._changes, function() {
          var query = db._changes
            .where('rev')
            .between(revision, maxRevision, false, true);
          return query
            .until(function() {
            ...
            .each(function(change) {
              ...
              if (change.type === CREATE) changeToSend.obj = change.obj;
              else if (change.type === UPDATE) changeToSend.mods = change.mods;
  ...
  function initGetTableObjectsAsChanges(
    db,
    node,
        ...
        .then(function() {
          if (limitReached) {
            ...
            if (state.tablesToUpload.length === 0) {
              ...
              return getChangesSinceRevision(
                state.localBaseRevision,
                MAX_CHANGES_PER_CHUNK - changes.length,
                brmcr.maxClientRevision,
                function(additionalChanges, partial, nodeModificationsOnAck) {
                  changes = changes.concat(additionalChanges);
                  nodeModificationsOnAck.dbUploadState = null;
                  return cb(
                    changes,
                    brmcr.remoteBaseRevision,
                    partial,
                    nodeModificationsOnAck
                  );
                }
              );
            } else {
              ...
              return getTableObjectsAsChanges(
                state,
                changes,
                db.table(state.currentTable).orderBy(':id')
              );
            }
          }
        });
  ...
  function initGetLocalChangesForNode(db, hasMoreToGive, partialsThreshold) {
    ...
    return function getLocalChangesForNode(node, cb) {
      ...
      var getChangesSinceRevision = initGetChangesSinceRevision(
        db,
        node,
        hasMoreToGive
      );
      var getTableObjectsAsChanges = initGetTableObjectsAsChanges(
        db,
        node,
        MAX_CHANGES_PER_CHUNK,
        getChangesSinceRevision,
      ...
      if (node.myRevision >= 0) {
        ...
        return getChangesSinceRevision(
          ...
          MAX_CHANGES_PER_CHUNK,
          brmcr.maxClientRevision,
          function(changes, partial, nodeModificationsOnAck) {
            return cb(
              changes,
              brmcr.remoteBaseRevision,
              partial,
        ...
        if (node.dbUploadState === null) {
          ...
          return db._changes.orderBy('rev').last(function(lastChange) {
            dbUploadState.localBaseRevision =
              (lastChange && lastChange.rev) || 0;
            var collection = db
              .table(dbUploadState.currentTable)
              .orderBy(':id');
            return getTableObjectsAsChanges(dbUploadState, [], collection);
          });
        } else if (node.dbUploadState.currentKey) {
          var collection = db
            .table(node.dbUploadState.currentTable)
            .where(':id')
            .above(node.dbUploadState.currentKey);
          return getTableObjectsAsChanges(
            Dexie.deepClone(node.dbUploadState),
            [],
            collection
          );
        } else {
  ...
  function initConnectProtocol(
    ...
    return function connectProtocol(node, activePeer) {
      /// <param name="node" type="db.observable.SyncNode"></param>
      var getLocalChangesForNode = initGetLocalChangesForNode(
        db,
        hasMoreToGive,
        protocolInstance.partialsThreshold
      );
        ...
        if (node.status !== newStatus) {
          ...
          node
            ...
            .then(function() {
              ...
              db.observable.broadcastMessage(
                'syncStatusChanged',
                { newStatus: newStatus, url: url },
                false
              );
            })
      ...
      function doSync() {
        ...
        return enqueue(
          ...
          function() {
            ...
            return getLocalChangesForNode_autoAckIfEmpty(
              node,
      ...
      function sendChangesToProvider(
        changes,
        remoteBaseRevision,
        partial,
        nodeModificationsOnAck
      ...
      function getLocalChangesForNode_autoAckIfEmpty(node, cb) {
        return getLocalChangesForNode(node, function autoAck(
          changes,
          remoteBaseRevision,
          partial,
          nodeModificationsOnAck
          ...
          if (
            changes.length === 0 &&
            'myRevision' in nodeModificationsOnAck &&
            nodeModificationsOnAck.myRevision !== node.myRevision
          ) {
            Object.keys(nodeModificationsOnAck).forEach(function(keyPath) {
              Dexie.setByKeyPath(
                node,
                keyPath,
                nodeModificationsOnAck[keyPath]
              );
            });
      ...
      function applyRemoteChanges(
        ...
        return enqueue(
          ...
          function() {
            ...
            return (partial
              ? saveToUncommittedChanges(remoteChanges, remoteRevision)
              : finallyCommitAllChanges(remoteChanges, remoteRevision)
      ...
      function continueUsingReactPattern(continuation) {
        ...
        function onChanges() {
          if (connectedContinuation) {
            changeStatusTo(Statuses.SYNCING);
            if (isWaitingForServer) changesWaiting = true;
            else {
              reactToChanges();
            }
          }
        ...
        function reactToChanges() {
          ...
          getLocalChangesForNode_autoAckIfEmpty(node, function(
            changes,
            remoteBaseRevision,
            partial,
            nodeModificationsOnAck
            ...
            if (changes.length > 0) {
              continuation.react(
                ...
                partial,
                function onChangesAccepted() {
                  Object.keys(nodeModificationsOnAck).forEach(function(
                    keyPath
                  ) {
                    Dexie.setByKeyPath(
                      node,
      ...
      function continueUsingPollPattern() {
          ...
          getLocalChangesForNode_autoAckIfEmpty(node, function(
            ...
            function onSuccess(continuation) {
              ...
              if (partial) {
                ...
                if (
                  !isNaN(continuation.again) &&
                  continuation.again < Infinity
                ) {
        ...
        if (hasMoreToGive.hasMoreToGive) {
          ...
          !isNaN(connectedContinuation.again) &&
          connectedContinuation.again < Infinity
        ) {
          changeStatusTo(Statuses.ONLINE);
          setTimeout(function() {
            if (connectedContinuation) {
  ...
  function initConnectFn(db, activePeers) {
    return function connect(
      ...
      function execConnect() {
        ...
        var connectProtocol = initConnectProtocol(
          db,
          protocolInstance,
          dbAliveID,
          options,
        ...
        var getOrCreateSyncNode = initGetOrCreateSyncNode(
          db,
          protocolName,
          url
        );
  ...
  function Syncable(db) {
    ...
    db.on('message', function(msg) {
      ...
      Dexie.vip(function() {
        if (msg.type === 'connect') {
          ...
          db.syncable.on.statusChanged.fire(
            msg.message.newStatus,
            msg.message.url
          );
        }
      });
          ...
          return Dexie.vip(function() {
            return db._syncNodes
              ...
              .toArray(function(connectedRemoteNodes) {
                return Promise.all(
                  connectedRemoteNodes.map(function(node) {
                    return db.syncable
                      .connect(
                        node.syncProtocol,
                        node.url,
                      ...
                      .catch(function(e) {
                        console.warn(
                          'Dexie.Syncable: Could not connect to ' +
                            node.url +
                            '. ' +
                            (e.stack || e)
                        );
    ...
    db.on(
      ...
      true
    ); // True means the ready event will survive a db reopen - db.close()/db.open()
    db.syncable = {};
